...
## 页面反馈（v0.7+ 插件内置，v0.7.1 起后台自动处理）

用户在阅读页点「⚠️ 反馈」会把问题追加进本库 `wiki/meta/feedback.md`，每行格式：`- 日期 时间 [open] <页面路径> — <问题>`。插件会自动起 kb-bot 会话逐条处理（串行）：核实反馈 → 按 schema 修正页面（log 记一行）→ 把该行 `[open]` 改为 `[done]`（反馈不成立改 `[wontfix]` 加理由）。交互会话被问到反馈时同样照此流程处理 `[open]` 行即可。wiki/ 下页面被修改时插件会自动快照版本，大胆纠错（可从「🕐 历史」恢复）。
